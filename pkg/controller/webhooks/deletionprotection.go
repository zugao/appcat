...
func checkManagedObject(ctx context.Context, obj client.Object, c client.Client, cpClient client.Client, l logr.Logger) (compositeInfo, error) {
	...
	if !ok || ownerKind == "" {
		...
		return compositeInfo{Exists: isDeletionProtected(obj), Reason: fmt.Sprintf("%s label not set, blocking deletion", runtime.OwnerKindAnnotation)}, nil
	}
	...
	if !ok || ownerVersion == "" {
		...
		return compositeInfo{Exists: isDeletionProtected(obj), Reason: fmt.Sprintf("%s label not set, blocking deletion", runtime.OwnerVersionAnnotation)}, nil
	}
